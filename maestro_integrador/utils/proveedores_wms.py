...
def create_supplier(bd_integracion, bd_principal):
    ...
    cur = conn.cursor()
    cur.execute("SELECT l10n_latam_identification_type_id, vat, name, email, phone, mobile, street, zip, city, state_id, country_id FROM proveedores_wms where id=0")
...
def update_supplier(bd_integracion, bd_principal):
    datosmain = integracion_data(bd_principal)
    # conexion API WMS
    common = xmlrpc.client.ServerProxy('{}/xmlrpc/2/common'.format(datosmain[5]))
    uid = common.authenticate(datosmain[1], datosmain[3], datosmain[4], {})
    ...
    conn = psycopg2.connect("host="+ HOST +" dbname=" + datosmain[2] + " user=" + USERNAME +" password="+ PASS +"")
    cur = conn.cursor()
    cur.execute("SELECT id, name, email, phone, mobile, street, zip, city, state_id, country_id FROM proveedores_wms WHERE id<>0")
    datospsql = cur.fetchall()
    for dato in datospsql:
        id = dato[0]
